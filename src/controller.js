...
async function fetchTweet() {
  await twit.get(
    "search/tweets",
    {
      q: "@LyricsOOCbot -from:@LyricsOOCbot -RT",
      count: 5,
      result_type: "recent",
    },
          ...
          if (!posted.includes(el.id_str)) {
            ...
            if (el.user.id != config.twitter_API.userId) {
              const text = el.text
                ...
                .replace(/(@\S+)/gi, "")
                .replace(/ +(?= )/g, "");
              await createMedia(text, {
                tweetId: el.id,
                user: `@${el.user.screen_name}`,
                tweetIdStr: el.id_str,
              });
            }
          } else {
...
const postRandom = async () => {
  const imageData = await lyricsUtil.createImage(
    undefined,
    artists.getRandomArtist()
  );
  ...
  return new Promise(async (resolve) => {
    return await twit.post(
      ...
      async (err, data, response) => {
        ...
        if (!err) {
          let params = {
            status: `${name} by ${artists}`,
            media_ids: [meta_params.media_id],
          };
          if (reply != undefined) {
            params = {
              ...params,
              in_reply_to_status_id: reply.tweetIdStr,
              in_reply_to_status_id_str: reply.tweetIdStr,
            };
...
async function uploadMedia(filename) {
  return new Promise(async (resolve) => {
    return await twit.postMediaChunked(
      { file_path: filename },
      async (err, data, response) => {
        if (data) {
...
async function postMedia(filename, name, artists, reply) {
  ...
  const _params = await createMetadata(
    filename,
    name,
    artists,
    reply,
