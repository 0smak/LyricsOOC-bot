...
const getValidLyric = async (query) => {
  let lyrics = "";
  let lyric = "";
  let attempts = 0;
  while (lyrics.length === 0 && attempts < 10) {
    lyrics = await geniusService.getLyrics(query);
...
async function generateImage(img, name, artists, lyric) {
  const filename =
    `/tmp/${name} ${artists} ${new Date().getTime()}.png`.replace(/\s+/g, "-");
  ...
  for (let i = 0; i < hits.length && !found; i++) {
    found =
      [
        ...
        "best album",
        "best song",
      ].find((w) => hits[i].result.full_title.toLowerCase().includes(w)) ===
      undefined;
...
const createImage = async (q = undefined, idArtist = -1) => {
  if (q || idArtist > -1) {
    ...
    const lyric = await getValidLyric(`${name} ${artists}`);
    if (idArtist != -1 && lyric?.includes("Lyrics for this song have yet")) {
      return createImage(q, idArtist);
    } else if (
      idArtist == -1 &&
      lyric.includes("Lyrics for this song have yet")
    )
      return null;
    console.log("generating image...");
    let filename = await generateImage(img, name, artists, lyric).then(
      (f) => f
    );
    console.log({ filename, name, artists });
